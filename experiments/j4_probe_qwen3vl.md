...
## 补遗（2026-07-24，J4c/J4d）

**pixel cap 修复**：vLLM V1 与 transformers 4.57 均忽略一切 max_pixels kwargs（engine 级/per-request/processor kwarg 全验证无效）→ 两 harness 改 PIL 预缩放（边取 32 倍数，jpeg q95 缓存）。capverify：HF ptid 178 / runner 618（native ~2600+）✓。

**DocVQA 真同像素表（600k px，官方 ANLS，n=200，全法 skip=0）**

| 方法 | ptid | ANLS |
|---|---|---|
| none | 610 | 0.951 |
| **FastV @25%** | 176 | **0.518** |
| **RBM pre @25%** | 176 | 0.424 |
| post≡VZ @25% | 176 | 0.251 |
| Pyramid canonical @62.5% | 393 | 0.878 |

→ **FastV 胜 RBM +9.4pp（同 ptid 176）**；RBM 胜 post +17.3pp；Pyramid 2.2× 预算近无损。FastV 至此 4 基准中胜 3（TextVQA +8.2 / DocVQA +9.4 / GQA +7.0），**RBM 唯一强项 OCRBench（0.580 vs FastV 0.155，+42.5pp）**。RBM native 分辨率 ANLS 0.465（高于 600k 的 0.424——分辨率亦助我方，同像素表保守）。

**Pyramid iso-25% [1.0,0.0,0.0,0.0]**（机制忠实退化 schedule，keep_equiv 0.25 验证）：全线崩（TextVQA 0.073 / GQA 0.305 / OCR 0.005）——第一段后清零全部图像 token 使后 3/4 层失视觉基础。入稿=同预算对照一点（25% 下 PyramidDrop 机制不可行 vs RBM 0.60/0.42/0.58 鲁棒）。

产物：runs/j4_baselines_hf/{j4c,j4d}_*.json + j4_cleanup.sh + j4d_docvqa_cap.sh。
