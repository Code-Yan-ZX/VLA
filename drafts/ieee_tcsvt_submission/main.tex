...
whether they should precede token selection is under-controlled. We compare two
iso-budget arms that score the same $2\!\times\!2$ units with query-blind L2 on
merger-input (pre) or merger-output (post) features, holding model and decoding
fixed. Across Qwen3-VL-8B, Qwen2.5-VL-7B, and InternVL3-8B, pre leads the
...
pre accuracy with identical selected-unit identities. We turn this finding into
...
\textbf{Contributions.}
First, we isolate a selection-level mechanism on Qwen3-VL: rank reshuffling,
text-stroke demotion, and ranking-swap control identify the kept set as the
lever (\S\ref{sec:mechanism}). Second, we establish a workload-conditioned
stage law across three models and two merger designs under the shared L2 family
and iso-budget control (\S\ref{sec:experiments}). Third, we derive minimal RBM,
map its trade-off against FastV, and show that five prespecified extensions do
not improve their stronger constituents (\S\ref{sec:fastv}, \S\ref{sec:negative}).
...
Across three tested models, pre-merger selection better preserves text-dense
accuracy; Qwen3-VL diagnostics indicate that its native merger rewrites
saliency in a text-hostile direction. RBM is therefore a robust default, not a
universal winner. It retains dense OCR better
...
We isolate visual-token selection stage under iso-model, iso-budget, and a
shared query-blind L2-magnitude family. Across the tested text-dense cells of three models
from two architecture families, pre-merger ranking is consistently stronger;
