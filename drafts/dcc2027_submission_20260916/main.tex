...
Visual-token reduction can operate inside the language model. FastV uses early-layer attention to select visual tokens~\cite{chen2024fastv}; PyramidDrop, FasterVLM, and SparseVLM explore layerwise or attention-based reduction~\cite{xing2024pyramiddrop,zhang2024fastervlm,zhang2024sparsevlm}. Their question-conditioned signals differ from RBM's image-only norm score. A comparison at equal final token count is informative about accuracy, but does not imply equal computation because the candidates can remain active through different numbers of layers.

A second line reduces or aggregates visual features before language processing. ToMe merges similar tokens~\cite{bolya2023tome}; LLaVA-PruMerge and VisionZip combine saliency and redundancy reduction~\cite{shang2024prumerge,yang2024visionzip}; TokenPacker learns a compact visual projector~\cite{li2024tokenpacker}. FitPrune estimates pruning allocation, and AdaptMerge uses language guidance~\cite{ye2024fitprune,islam2025adaptmerge}. These methods motivate studying representation and selection jointly, but our experiments do not rank this entire design space.

We focus on architectures with native spatial compression: Qwen2.5-VL, Qwen3-VL, and InternVL3~\cite{bai2025qwen25vl,bai2025qwen3vl,zhu2025internvl3}. RBM retains their native operators and selects complete input units. Post-L2 is a diagnostic baseline from the same feature-magnitude family; FastV supplies a stronger attention-based comparison. The resulting evidence concerns these model implementations and workloads, rather than a claim that downstream selectors generally assume rank preservation.
...
We also examine a hybrid, RankBridge, that protects $q=\operatorname{round}(\alpha k)$ RBM-ranked positions and fills the remaining $k-q$ positions from FastV's ranking, excluding protected positions. Its prior weight $\alpha=0.2$ was selected from $\{0.1,0.2,0.3\}$ in a development gate. All candidates remain available until the FastV selection layer, $K=3$. Thus the hybrid's equal final token count does not give it RBM's earlier sequence reduction.

Table~\ref{tab:bridge} reports the resulting exploratory Qwen3-VL probe. TextVQA, DocVQA, and GQA use 200 examples each. OCRBench has 181 jointly evaluable examples after 19 common loading skips; its row reports accuracy over those 181 examples. RankBridge improves the observed FastV scores by 0.5--3.2 percentage points, but trails RBM on OCRBench by 11.6 points. It fails the development objective of remaining within one point of RBM there. The available results suggest complementary signals, not a validated universal hybrid. We make no claim of a statistically significant improvement or of an independently held-out confirmatory test.

\begin{table}[t]
\centering
\caption{\label{tab:bridge}Exploratory Qwen3-VL hybrid probe at 25\% final retention. $n$ is the common effective sample size; OCRBench is accuracy on the 181 completed samples. Bold marks the highest observed score.}
\small
\begin{tabular}{lrrrr}
\toprule
Benchmark & $n$ & RBM & FastV-k3 & RankBridge\\
\midrule
TextVQA & 200 & .5967 & .7633 & \textbf{.7950}\\
DocVQA & 200 & .4239 & .5863 & \textbf{.6023}\\
OCRBench & 181 & \textbf{.5801} & .4586 & .4641\\
GQA & 200 & .4150 & .5050 & \textbf{.5100}\\
\bottomrule
\end{tabular}
\end{table}

\Section{Diagnostics and Interpretation}
The operational Qwen3-VL pre/post ranks differ substantially. On the diagnostic DocVQA, TextVQA, and GQA samples, Spearman correlations are .137, .332, and .360, and top-25\% Jaccard overlaps are .180, .243, and .278. These observations show that representation choice changes the selected set. Because the operational ranks use different feature depths, these diagnostics alone do not isolate the main merger's contribution.

Rank shifts also associate with local edge energy. Correlations between post-minus-pre rank and Sobel energy are +.439, +.155, and +.036 for the same task order. On DocVQA, RBM-only units have mean edge energy .641 versus .124 for Post-L2-only units; 92\% versus 35\% exceed their image's median. Sobel energy responds to borders, texture, and object contours as well as text. The measurements therefore establish an edge association, not text-specific demotion or a causal explanation of answer accuracy. Figure~\ref{fig:examples} illustrates different spatial allocations without serving as aggregate evidence.

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{figs/fig2_vector.pdf}
\caption{\label{fig:examples}Illustrative OCRBench selections at 25\% retention. RBM, Post-L2, and FastV-k3 preserve different spatial regions. These examples visualize selection behavior; quantitative comparisons use the separately specified evaluation sets.}
\end{figure}

A ranking-swap check fixes the post-merger computation but substitutes RBM's kept identities. On the verified Qwen3-VL path, its kept-set overlap with RBM is 1.000, retained representations are byte-identical, and predictions agree. This verifies path equivalence when the set is fixed and excludes altered computation on surviving units as the explanation for that check. It does not establish that norm or edge energy identifies causally relevant text. The analogous Qwen2.5-VL path retains an ordering confound; InternVL3 supplies accuracy-level replication, without an equivalent byte-exact attribution claim.

\Section{Cost and Limitations}
The retained-unit fraction $k/N$ measures the final visual sequence budget. It is neither an encoded bit rate nor a sufficient measure of inference cost. RBM, Post-L2, and FastV differ in the stage at which sequences are reduced. In particular, FastV and RankBridge process all candidate visual tokens through the initial language-model layers. Native image resolution, output length, batching, and backend also affect wall-clock performance. The method does not eliminate vision-transformer work before the scoring tap, and the Pre-final control deliberately executes the full merger streams.

On one A40, a Qwen3-VL TextVQA batch of 200 examples reaches 6.91 requests/s with RBM at 25\% retention, versus 4.11 without compression and 6.89 with Post-L2. The vLLM run uses native image resolution, at most eight concurrent sequences, greedy decoding capped at 32 output tokens, and one untimed warmup; timing surrounds batched generation and excludes engine construction and initial image loading. A separate five-run, single-example proxy averages .23 seconds for RBM and .36 seconds without compression. The throughput result supports an end-to-end gain for this workload, while the nearly equal RBM/Post-L2 rates do not show a selector-specific speed advantage. The one-example proxy is not a serving-latency distribution.

Our strongest controlled conclusion is the full-split Qwen3-VL final-input comparison, with the stated score-aggregation limitation. The three-model operational matrix extends the empirical pattern across native merger designs, but does not establish a universal causal mechanism. Post-L2 is a diagnostic baseline; the FastV comparisons show that RBM is not uniformly strongest even among the tested methods. The hybrid remains exploratory, and shared loading failures limit the scope of the HF OCRBench results. Broader selector comparisons and direct text-region interventions would be needed for general claims about competitiveness or semantic mechanisms.
