...
\caption{\label{fig:ocr-cases}Selected OCRBench cases at 25\% retention. Blue cells are retained visual units; checkmarks indicate correctness under the official answer rule for these examples. The cases illustrate different spatial allocations and do not replace the full-split comparison.}
\end{figure}
...
\begin{figure}[t]
\centering
\includegraphics[width=.80\textwidth]{figs/fig3_fullsplit_controls.pdf}
\caption{\label{fig:controls}Full-split Qwen3-VL results at 25\% retention. Early-tap RBM is operational; Pre-final and Post-L2 form the final-input control. OCRBench is normalized by 1,000.}
\end{figure}

\SubSection{Comparison with attention-based selection}
A same-harness full-OCRBench comparison evaluates RBM and FastV-k3 at a shared four-million-pixel cap and a 25\% final budget (Table~\ref{tab:fastv}); Fig.~\ref{fig:ocr-cases} provides separately audited selected cases. Both methods use native positions, greedy decoding, and at most 32 generated tokens. The Qwen3-VL and Qwen2.5-VL runs encounter 79 and 88 common context or memory failures, respectively. These exact shared IDs receive zero in the scores out of 1,000; the completed paired sets contain 921 and 912 examples. RBM leads by 146 and 87 points. These HF scores are a separate experiment and must not be substituted into the vLLM matrix.
...
\begin{figure}[t]
\centering
\includegraphics[width=.88\textwidth]{figs/fig4_vector.pdf}
