...
\hypersetup{
  pdftitle={Rank Before You Merge: Training-Free, Merger-Aware Visual Token Compression for Vision-Language Models},
  pdfauthor={Zhengxing Yan, Hao Sun, Chen Guo, Jianpeng Hu},
...
\title{\large\textbf{Rank Before You Merge: Training-Free, Merger-Aware
...
\begin{abstract}
Most VLM token selectors rank outputs of a native spatial merger, implicitly
assuming that merging preserves which evidence is salient. We show that a
learned merger can instead rewrite feature-norm rankings and demote units that
contain text strokes. Rank-Before-Merge (RBM) addresses this operator-order
failure by ranking complete native units before the original merger, under a
fixed per-image budget and unchanged language-model interface. At 25\%
retention, RBM outperforms matched post-merger L2 selection in all nine
text-dense model--benchmark pairs across Qwen3-VL, Qwen2.5-VL, and InternVL3,
by 11.0--38.4 percentage points and 235--432 OCRBench points. Rank, edge-energy,
and swap controls trace the gap to merger-induced saliency rewriting. A
pre-merger prior also improves FastV-k3 on four locked Qwen3-VL probes, whereas
the GQA reversal marks the workload boundary. RBM is training-free, adds no
parameters, and raises measured Qwen3-VL throughput by 68\% at 25\% retention.
Code:
\href{https://github.com/Code-Yan-ZX/qrbm-vlm}{github.com/Code-Yan-ZX/qrbm-vlm}.
\end{abstract}

\Section{Introduction}

Modern VLMs expose hundreds to tens of thousands of visual tokens to the
language model. Retaining 25\% of Qwen3-VL's visual tokens lowers latency on our
A40 from 0.36 to 0.23 seconds and raises throughput by 68\%. Compression must
therefore decide which evidence should occupy the limited rate.

Most selectors rank tokens after a native spatial merger combines neighboring
patches. This order assumes that merging preserves the evidence that would have
ranked highest before merging. A learned merger can instead reverse the
top-$k$ order, hiding answer-bearing regions before downstream selection begins.
The representation on which top-$k$ acts thus changes task distortion even at
the same rate. We ask whether ranking before the merger reduces this distortion
when the budget, merge units, and downstream model are fixed.

Rank-Before-Merge (RBM) makes one intervention: it scores complete
$2\!\times\!2$ units at the merger input, then sends an exact per-image budget
through the unmodified merger and language-model interface
(Fig.~\ref{fig:overview}). Its units, positions, stream identity, and interface
follow the deployed graph; we call this contract \emph{machine-centric}.

\begin{figure}[t]
  \centering
  \includegraphics[width=\figurewidth]{figs/fig1_user_20260915_v2.png}
...
At 25\% retention, RBM improves all nine text-dense full-split comparisons
across three VLMs and two mergers; GQA favors post-merger selection. Rank and
token-survival analyses show that the merger rewrites the selected set and
disproportionately demotes text-stroke-rich units. A locked RankBridge probe
also shows that pre-merger rank complements FastV attention.

We contribute: (1) an operator-order formulation identifying when selection
commutes with a native merger; (2) RBM, a training-free compressor that changes
only selection order; and (3) matched full-split results and controls that
establish its accuracy, mechanism, efficiency, and workload boundary.

\Section{Related Work}

Visual-token reduction may act inside the LLM through attention or progressive
pruning~\cite{chen2024fastv,xing2024pyramiddrop,zhang2024fastervlm,
zhang2024sparsevlm}, or at the encoder output through [CLS] attention, graph
propagation, fitted predictors, and multiple cues
...
Merging also changes representation. ToMe, VisionZip, and AdaptMerge use
matching, dominant-token preservation, and language guidance
~\cite{bolya2023tome,yang2024visionzip,islam2025adaptmerge}; TokenPacker learns
a compact projector, while GlimpsePrune, VScan, and AnchorPrune adapt reduction
to inputs or model families
~\cite{li2024tokenpacker,zeng2025glimpseprune,zhang2025vscan,
oh2026anchorprune}. These methods motivate treating a selector's input
representation as part of its compression rule.

Recent selectors move reduction earlier or vary it across tokens and layers
~\cite{gao2026quietprune,wang2026pixelprune,li2026transprune,
chen2026v2drop,sun2026hiloprune}; optimize coverage, reconstruction, or
sensitivity~\cite{ma2026apet,xu2026score,du2026coin,kim2026zooprune,
lee2026spare}; and model semantics, information flow, or head importance
~\cite{wang2026metacompress,song2026evocomp,sun2026ifprune,zhu2026hawk}.
Layerwise, object-centric, local/global, and hybrid designs preserve evidence in
other ways~\cite{wang2026informationhorizon,chen2026alvts,li2026crisp,
lei2026core,li2026aot,zhang2026hybrid}. Document pruning and provenance audits
expose OCR-specific failures~\cite{choi2026docprune,liu2026provenance}, while
LMMs-Eval and EffiVLM-Bench support evaluation
...
\Section{Compression Before a Native Merger}

\SubSection{When does selection commute with merging?}
...
\label{eq:sets}
\end{equation}
With deterministic tie breaking, selection commutes with merging at budget
$k$ exactly when $K_{\mathrm{pre}}=K_{\mathrm{post}}$. A sufficient condition
for agreement at every budget is $b_i=g(a_i)$ for a strictly increasing $g$.
We measure the violation at budget $k$ by
\begin{equation}
...
Table~\ref{tab:main} isolates the operator-order effect within query-blind L2
scoring.
...
Three Qwen3-VL diagnostics explain the operator-order gap
(Fig.~\ref{fig:mechanism}). Pre/post ranks correlate weakly: Spearman
correlations are 0.137, 0.332, and 0.360, while top-25\% Jaccard overlaps are
0.180, 0.243, and 0.278 on DocVQA, TextVQA, and GQA. The merger changes the
top-$k$ set, not only score scale.
...
A native merger changes the ranking on which later compression depends. RBM
therefore selects complete units before merging and leaves the surviving
computation unchanged. At equal rates, it preserves substantially more
text-dense evidence across three VLMs and two merger designs while accelerating
inference. The GQA reversal bounds the claim: pre-merger rank is a strong
text-rich signal, not a universal replacement for question-conditioned
selection. Selection stage and native topology should be treated as part of
VLM rate--distortion design.
