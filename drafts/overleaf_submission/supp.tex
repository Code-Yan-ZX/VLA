...
no-regression-across-four-benchmarks gate passes. Source:
\path{experiments/freq_aware/results.json} +
\path{experiments/freq_aware/grid.json} (\path{scripts/freq_adaptive_driver.py}).
...
\path{experiments/adaptive_stage/results.json} +
\path{experiments/adaptive_stage/grid.json}. Workload-signal analysis:
\path{scripts/adapt_discriminator_probe.py}.

\noindent\textbf{S4c.3 Diversity-constrained RBM} (exploratory extension):
Diversity-RBM traverses an importance-ranked candidate pool and suppresses a
candidate when its cosine similarity to an already selected unit exceeds a
fixed threshold; Adaptive-Diversity-RBM conditions that threshold on the
image's participation-ratio effective rank. Both variants retain exactly the
same number of units as RBM. Table~\ref{tab:supp_diversity} reports the
Qwen3-VL-8B $n=500$ probe. We convert the runner's prune ratio $r$ to retained
fraction $\kappa=1-r$ to match the body. The probe covers only mild compression
($\kappa\in\{75\%,50\%\}$), one model, and fixed evaluation slices.
...
\begin{table*}[t]
  ...
  \begin{tabular}{@{}l c r r r r@{}}
    \toprule
    Benchmark & Retention $\kappa$ & RBM & FastV & Diversity-RBM &
    Adaptive-Diversity-RBM \\
    \midrule
    TextVQA   & 75\% & 0.745 & 0.665 & \textbf{0.748} & 0.741 \\
    TextVQA   & 50\% & 0.673 & 0.387 & 0.673 & \textbf{0.681} \\
    DocVQA    & 75\% & 0.668 & \textbf{0.674} & 0.667 & 0.668 \\
    DocVQA    & 50\% & \textbf{0.621} & 0.549 & 0.612 & 0.611 \\
    OCR-Bench & 75\% & 0.838 & --- & \textbf{0.840} & 0.834 \\
    OCR-Bench & 50\% & 0.822 & 0.498 & 0.828 & \textbf{0.832} \\
    GQA       & 75\% & 0.486 & \textbf{0.506} & 0.480 & 0.496 \\
    GQA       & 50\% & 0.484 & 0.500 & \textbf{0.506} & 0.502 \\
    \bottomrule
  \end{tabular}
  \caption{Exploratory diversity-constrained selection on Qwen3-VL-8B
  (official metrics, $n=500$). Bold marks the largest point estimate in each
  row, not a significant winner. Against the stronger available RBM/FastV
  incumbent in each row, paired bootstrap intervals for both diversity variants
  include zero; this is inconclusive evidence, not an equivalence result. The
  missing OCR-Bench FastV cell at 75\% retention is due to a runtime failure.
  This single-model mild-compression probe is not a headline SOTA comparison.}
  \label{tab:supp_diversity}
\end{table*}
...
200/200 per benchmark for pre-final vs post). Source:
\path{experiments/p0-3_prefinal_control.md}; per-sample JSONs in
\path{runs/qwen3_prefinal_control/}.
...
\begin{itemize}
  \item \textbf{Qwen3-VL-8B}: the first deepstack merger has index 0 and the
    ...
    diagnostic \path{mask_computed_at} $=$ \texttt{'deepstack\_0'} on every
    run. Source:
...
\begin{table*}[h]
  ...
  \begin{tabular}{@{}p{3.0cm} c p{4.6cm} p{4.6cm}@{}}
    ...
    Table 2 FastV-k3                  & 8            & \path{runs/r2_same_scope/r2b_*.json} & \path{experiments/r2b_fastv_k3.md} \\
    Table 2 RBM same-scope            & 5            & \path{runs/r2_same_scope/r2c_*.json} & \path{experiments/r2c_rbm_scope.md} \\
    Table 4 cascade gate              & 24/24        & \path{runs/cascade/gate_*.json} + \path{gate_result.json} & \path{experiments/cascade_gate.md} \\
    Table 5 efficiency                & 7 configs    & \path{runs/full_matrix/efficiency/*.eff.json} & \path{experiments/j6_efficiency.md} \\
