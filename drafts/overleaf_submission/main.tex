...
\usepackage{multirow}     % Tables 1--3 model-column row spans
...
GQA remains near zero and not significant here ($-0.4$~pp), whereas
...
mean retention $0.625 = 2.5\times$ ours); at the tested iso-25\%
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}l r r r l@{}}
    ...
    OCR-Bench (full; 4M cap)     & 1000  & 413             & \textbf{559} & \textbf{+146 R} \\
    \midrule
    ...
    OCR-Bench (full; 4M cap)     & 1000  & 295             & \textbf{382} & \textbf{+87 R} \\
    \bottomrule
  \end{tabular}
  ...
  OCRBench scores and deltas are reported on the /1000-point scale.
...
measured on \textbf{Qwen3-VL-8B}. M1/M2 use deterministic seed-0 samples:
OCRBench has $n=199$, while TextVQA, DocVQA, and GQA each have $n=64$; M3 uses
$n=200$ with the same invocation as the headline cells. We then state the
architectural scope.
...
(Qwen3-VL and Qwen2.5-VL $\times$ DocVQA and TextVQA; $n = 32$; independent
random 25\%-retained sets have expected Jaccard $\approx 0.143$). The two paths
select \emph{exactly the same units}. The
