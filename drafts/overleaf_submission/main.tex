...
% FIG 1 --- frontispiece: one OCRBench example, three zones, 7.0x3.6 inches
\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{figs/fig1.pdf}
  \caption{\textbf{Saliency should be measured before information mixing.}
  The left panel shows an OCR-sensitive airport-board region for the query
  ``which counter is boarding?'' (ground truth: \texttt{A105-108}). The insight
  panel isolates the ordering difference: FastV-style selection first merges
  local visual units and then ranks them, whereas Rank-Before-Merge (RBM)
  ranks raw $2\!\times\!2$ units before the native merger. The two method
  branches are otherwise symmetric and use the same budget,
  $\kappa=25\%$. On this measured OCRBench case, FastV retains the wrong
  counter (\texttt{A115-126}), while RBM retains the answer-bearing units and
  returns \texttt{A105-108}.}
  \Description{A minimalist vector overview has four labeled regions: Problem,
  Insight, Method, and Evidence. It compares Merge-then-Rank with
  Rank-then-Merge, shows symmetric orange RBM and blue FastV-style branches at
  kappa 25 percent, and reports the incorrect A115-126 versus correct A105-108
  answers on an airport information board.}
  \label{fig:fig1}
\end{figure*}
