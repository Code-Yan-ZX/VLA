...
\begin{figure*}[t]
  ...
  \caption{\textbf{Rank before the native merger preserves answer-bearing text that later selection can no longer recover.}
  On one OCRBench airport-board example (ground truth: \texttt{A105-108}),
  Qwen3-VL-8B-Instruct retains 25\% of the visual tokens, or 69 final tokens,
  in every arm. \emph{Left:} the answer region in the input. \emph{Middle:}
  RBM ranks raw $2\!\times\!2$ units before the native merger, whereas
  Post-L2 ranks already merged tokens. \emph{Right:} identically cropped
  evidence regions with each method's own measured retained-token mask. RBM
  preserves the counter identifier and answers correctly; Post-L2 and FastV-k3
  do not. This example illustrates the OCR-heavy regime rather than a universal
  ordering: FastV remains stronger in several TextVQA and DocVQA settings.}
  \Description{An airport departure board example compares selection before
  and after the native merger. RBM retains the region containing counter
  A105-108 and answers correctly; Post-L2 and FastV-k3 answer incorrectly.}
  \label{fig:fig1}
\end{figure*}
