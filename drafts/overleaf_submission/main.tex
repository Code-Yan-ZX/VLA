...
\textbf{The CVPR 2026 frontier.} Recent work broadens token reduction beyond
...
the need to localize the intervention stage, but none performs our matched
pre/post test at a frozen model's native spatial merger.
...
(full values in Supplementary Table~S3, $n=200$).
...
\textbf{Exploratory diversity-constrained selection.} A subsequent,
non-prespecified probe augments RBM with importance-ordered cosine NMS
(Diversity-RBM), or conditions the NMS threshold on image effective rank
(Adaptive-Diversity-RBM), following recent diversity-aware
selectors~\cite{fang2026prunesid,baek2026agilepruner}. On the same Qwen3-VL
$n=500$ slices at 75\% and 50\% retention, each fixed variant trades small wins
and losses against plain RBM; neither establishes an improvement over the
stronger per-cell RBM/FastV incumbent. The clearest conditional result is
TextVQA at 75\% retention, where Diversity-RBM scores 0.748 versus 0.665 for
FastV but only 0.745 for RBM. We therefore treat diversity as a competitive
mild-compression extension, not a new default or a headline comparison
(Supplementary~S4c.3).
