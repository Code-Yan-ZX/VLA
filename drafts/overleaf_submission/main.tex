...
whether they should precede token selection is under-controlled. We compare
iso-budget arms that rank the same $2\!\times\!2$ units with query-blind L2 on
pre- or post-merger representations, holding model and decoding fixed; a
separate Qwen3-VL matched-depth control isolates the main-merger boundary.
Across Qwen3-VL-8B, Qwen2.5-VL-7B, and InternVL3-8B, pre leads the
matched L2-family post arm on all nine official TextVQA, DocVQA, and OCRBench cells at
25\% retention, by $+11.0$ to $+38.4$ percentage points and $+298$ to $+432$
OCRBench points. A full-split matched-depth control at the main-merger boundary
retains the text-dense effect but finds a $-5.6$-pp counter-direction on GQA.
...
and OCR preservation, but post-merger ranking leads by $5.6$~pp on full-split
GQA; the result is workload-conditional and widens with compression depth
...
\textbf{Closest concurrent boundaries.} IVC-Prune and OccamToken are
prompt/query-aware; GMC transports discarded decoder messages; and GOTS
optimizes query-blind subset complementarity
\cite{sun2026ivcprune,li2026occamtoken,qian2026gmc,ling2026gots}. N\"uwa and
DUET-VLM coordinate encoder- and LLM-side reduction, HTC-VLM learns a hybrid
bottleneck, and Reroute changes token lifetime
\cite{huang2026nuwa,singh2026duetvlm,zhang2025htcvlm,yang2026reroute}.
These alternatives change the scoring rule, representation, or token lifetime;
none is a matched before/after test of a frozen native spatial merger under one
shared ranking rule. Our narrower design point leaves that merger untouched,
ranks its input $2\!\times\!2$ units with query-blind L2 saliency, and couples
the controlled stage comparison with mechanism evidence (M1--M3).
...
\begin{table*}[t]
  ...
  \caption{Official full-split metrics ($n = 5000 / 5349 / 1000 / 12578$),
    ...
    only significant post-stage leads in this table (Qwen GQA). The full-split
    matched-depth Qwen3 control in \S\ref{sec:qwen} also favors post on GQA,
    establishing a workload-conditioned counterexample rather than a universal
    pre-stage benefit. InternVL3 uses a different merger and
  ...
  \begin{tabular}{@{}ll r r r l r r@{}}
    ...
    \multirow{4}{*}{Qwen2.5-VL-7B}
      ...
      & OCR-Bench (/1000)   & 817   & \textbf{480}   & 182   & \textbf{+298 pts} & 335   & 67 \\
  ...
  token counts. The Qwen2.5-VL OCR-Bench @25\% pair is the matched 4~M-pixel
  rerun (480/182; mean tokens 229.6 in both arms, 0 skips); its native-resolution
  none anchor is descriptive rather than an iso-configuration comparator.
  (e)~Primary inference uses paired-bootstrap 95\% CIs and
...
mergers untouched (Qwen3-VL, greedy, $\kappa = 0.25$, L2, official full
  splits; iso-token and sample-id equality verified in every cell). The only
...
Pre-final still beats post by $+27.7$~pp on TextVQA, $+4.6$~pp on DocVQA, and
$+235$ OCRBench points. Paired 95\% CIs exclude zero, and sign-flip tests remain
significant after Holm correction over the four controls. The full table is
in Supplementary~S7b. The text-dense
...
\textbf{Object-centric GQA --- a workload-conditioned counterexample.} On the
full split, pre-final scores 0.4207 versus 0.4771 for post: $-5.64$~pp, paired
95\% CI $[-6.42,-4.86]$~pp, sign-flip $p=5\times10^{-5}$ (Holm-adjusted
$p=5\times10^{-5}$), $n=12578$, with identical sample ids and token counts.
The earlier $n=200$ control happened to give $0.450=0.450$; we retain it only
as a superseded small-subset estimate, not as the claim-level result. Thus
post-merger ranking can be preferable for object-QA, while pre-final remains
stronger on all three full-split text-dense benchmarks. The original layer-8
pre tap partly offsets the GQA loss (0.449 versus 0.421 for pre-final) but does
not reverse the full-split post lead. This separates the stage finding from a
universal ranking claim and is consistent with GQA's near-null text
directionality (M2 $\rho=0.04$; \S\ref{sec:mechanism}).
...
Qwen3-VL's full-split matched-depth control has a significant $-5.6$-pp
pre-final$-$post difference (\S\ref{sec:qwen}). There is no text-dense crossover
at either family's native imaging configuration. These
...
accuracy, and Qwen3-VL diagnostics indicate merger-induced saliency rewrites.
RBM is nevertheless a robust default, not a universal winner: it retains dense
OCR better than the compared FastV settings ($+16.0/+8.5$~pp), while FastV
leads on several scene-text/object-centric cells (Table~\ref{tab:tab3}) and the
matched-depth full-split GQA control favors post.

\textbf{Scope.} Byte-exact M3 causality is established only on Qwen3-VL;
Qwen2.5-VL retains an ordering confound, InternVL3 supplies bounded
accuracy/ranking-swap support, and models without a native merger are out of
scope. M1/M2 use a Sobel proxy ($n=64$ each) and M3 uses $n=200$. Holm correction
covers the nine prespecified 25\%-retention text-dense contrasts and, separately,
the four full-split matched-depth controls; other analyses are secondary or
exploratory. FastV accuracy anchors use HF eager, so throughput comparisons are
vLLM-only. Cross-model absolute scores are also confounded by marked sample and
pixel-cap differences. Low-resolution Qwen2.5-VL reverses the stage direction,
and RBM remains lossy versus no compression on InternVL3 DocVQA/OCR despite
beating post.
...
Under one shared L2 ranking rule, pre-merger selection consistently wins on the
tested text-dense cells; Qwen3-VL controls connect the gap to merger-induced
rank changes. RBM is thus a training-free OCR-oriented default, not a universal
winner: FastV leads on several other settings and matched-depth GQA favors post.
RBM adds no stage-specific serving cost, and five prespecified extensions fail
