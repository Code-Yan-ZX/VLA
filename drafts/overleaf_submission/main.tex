...
% FIG 1 --- real qualitative case-level comparison (Qwen3-VL-8B-Instruct, 3
% examples x 3 methods; wired from figs/fig1.pdf, no proxy).
\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{figs/fig1.pdf}
  \caption{\textbf{Selected cases where RBM preserves text that baselines miss.}
    Three cases where \textbf{Qwen3-VL-8B-Instruct} under greedy decoding at
    $\kappa = 25\%$ visual-token retention produces the correct answer under
    our pre-merger RBM selector while at least two displayed baselines
    (FastV, Post-L2) fail. Every row of an example is evaluated on the
    \emph{identical} final visual-token count ($N_{\text{ptid}}$; 218, 175,
    219 for the three rows). The red box marks the answer-bearing region in
    the source image; the tinted blocks are the pre-merger RBM retained units
    (FastV and Post-L2 retained-token indices were not stored in the source
    runs, so the pre-merger mask is shown for visual reference only; the
    contrast is in the \emph{answer}, not in the overlay). Answers are
    generation-truncated (max\_tokens = 32, greedy). \emph{Top}: TextVQA
    34863, "does it say happy birthday?" --- RBM~\checkmark, FastV/Post-L2
    read the LED scoreboard as `no'. \emph{Middle}: DocVQA 52297, "What are
    the dates for the conference?" --- RBM~\checkmark, FastV drops the day
    to `July 1996', Post-L2 corrupts the year to `July 1972'. \emph{Bottom}:
    OCRBench ocr0804, "value for CIRCULATION DATES?" --- RBM~\checkmark,
    FastV drops the year, Post-L2 reads a different date. We do not claim
    RBM is universally better; on text-dense workloads (TextVQA, DocVQA,
    OCRBench) the pre-merger selector protects small/dense text that the
    2$\times$2 merger would otherwise average into background, while on
    object/scene workloads (e.g.\ GQA) post-merger selection can win. The
    pool sizes from which these three cases were drawn (all constrained to
    matched $\kappa$ and same-image pairing) are 24, 37, and 9 respectively
    for TextVQA, DocVQA, and OCRBench.}
  \label{fig:fig1}
\end{figure*}
