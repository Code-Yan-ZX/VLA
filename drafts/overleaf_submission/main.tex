...
same-scorer post arm on all nine official TextVQA, DocVQA, and OCRBench cells at
25\% retention, by $+11.0$ to $+38.4$ percentage points and $+293$ to $+432$
OCRBench points. A matched-feature control retains the text-dense effect at the
main-merger boundary but detects no GQA difference.
...
learned scorer or query input. The gains are against the same-scorer post arm,
...
per-unit, the merged token for any kept unit is
bit-identical under either stage; RBM changes only which units reach the merger.
...
iso-budget, and the same L2 selector, pre leads post by $11.0$--$38.4$~pp on
...
differs. A matched-feature pure-stage control retains the TextVQA/DocVQA effect
...
This same-scorer result is distinct from the query-conditioned FastV comparison:
...
stage law across three models and two merger designs under the same L2 selector
...
model's native spatial merger under iso-model, iso-token, and the same
functional L2 selector.
...
comparisons without the matched stage-only control used here.
