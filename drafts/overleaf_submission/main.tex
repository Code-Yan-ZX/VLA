...
% Conference metadata is supplied by ACM for the camera-ready version.
...
their effect on later token selection has not been isolated. We compare the same functional, query-blind L2 selector family immediately before and after
the native merger, holding model, per-image token budget, and decoding fixed. Across Qwen3-VL-8B,
...
and object-centric settings. Six prespecified extensions do not improve over
...
iso-budget, and the same functional L2 selector, pre-merger selection leads by
...
text-stroke demotion, and a ranking-swap control identify the kept set as the
selection-level lever (\S\ref{sec:mechanism}). Second, we establish a
workload-conditioned stage law across three models and two merger designs under
the same functional L2 selector and iso-budget control (\S\ref{sec:experiments}). Third, we derive the deliberately minimal
...
and the same functional L2 selector.
...
Qwen3-VL mechanism evidence (M1--M3) explaining why this stage can dominate on
...
\begin{itemize}
  ...
  \item \textbf{Post stage (contrast).} Either score the $N$ \emph{merged} units
    ...
    contrast, using the same functional L2 selector as the pre arm; the
...
same functional, query-blind L2 selector family as the pre arm, at an identical
...
accuracy-level replications show that the same functional L2 selector can favor
...
\textbf{Exploratory learned boundary scorer.} A subsequent held-out probe uses
a small MLP to map pre-merger L2,
edge-energy, local-variance, position, and neighborhood features to the
post-merger importance ordering. The scorer successfully distills that ordering
offline, but this is the wrong teacher for text-dense selection: on disjoint
held-out samples at 25\% retention it changes TextVQA from 0.595 to 0.435
($-16.0$~pp), while improving GQA from 0.515 to 0.550 ($+3.5$~pp).
Thus the learned model is not an RBM upgrade. Instead, the task-level failure
provides an independent intervention consistent with M2: learning the
post-merger ordering also learns its demotion of text-stroke-rich units
(Supplementary~S4c.3).

These results bound, rather than close, the tested design space: image-only
signals cannot access the query-dependent oracle headroom, cascading discards
evidence needed by the second selector, and distilling a post-merger teacher
reproduces its text-dense failure mode. We therefore freeze plain RBM as the
robust text-dense/OCR default and select a FastV-class method for
scene-text/object-centric traffic; within the tested configurations, routing,
cascading, and learned post-order imitation offer no consistent benefit.
...
$n=200$; the GQA null has no equivalence margin, and multiple comparisons have
...
We isolate visual-token selection stage under iso-model, iso-budget, and the
same functional L2 selector. Across the tested text-dense cells of three models
