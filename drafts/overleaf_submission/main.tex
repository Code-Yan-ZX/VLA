...
egin{figure*}[t]
  ...
  \caption{	extbf{Rank before the native merger preserves answer-bearing text that later selection can no longer recover.} A single OCRBench example (	exttt{ocr0422}, airport information board; Q: \emph{which counter is boarding?}; GT: 	exttt{A105-108}) under Qwen3-VL-8B-Instruct, greedy decoding, 25\% final visual-token retention; all three arms share the identical final token count of 69. \emph{Input} (left): the real image with one red evidence box around the 	exttt{A105-108 \dots boarding} region. \emph{Where to rank} (middle): the controlled stage contrast --- 	extbf{RBM (ours)} ranks the raw 2$	imes units \emph{before} the native merger, then keeps 25\%, then runs the unmodified native merger (amber); 	extbf{Post-L2} runs the native merger first, then ranks the merged tokens, then keeps 25\% (blue). The only emphasised phrase is \emph{rank before information is mixed}. \emph{Evidence} (right): two identically cropped evidence zooms with the method's real retained-token mask overlaid as square pixel blocks; RBM reads 	exttt{A105-108} and is marked Correct; Post-L2 cannot recover the answer and is marked Incorrect. 	extbf{FastV-k3} appears only as one compact third line: 	exttt{no counter boarding} --- Incorrect. Ground truth is printed once: 	exttt{A105-108}. Footer: 	exttt{25\% retained \,|\, identical final token count: 69} plus the small context anchor 	exttt{OCRBench: RBM +16.0 pp vs FastV-k3}. The retained-token masks for Post-L2 and FastV-k3 were not stored in the source runs, so the pre-merger RBM mask is shown for visual reference in all three evidence rows; the contrast is in the \emph{answer}, not in the overlay. The example was selected from the 9 strict three-arm flips in 	exttt{drafts/figures/camera\_ready/contact\_sheet\_manifest.json} where RBM is correct and both FastV-k3 and Post-L2 are wrong. We do not claim RBM is universally better; it preserves text on text-dense workloads while FastV remains stronger on several TextVQA/DocVQA settings.}
  \label{fig:fig1}
\end{figure*}
