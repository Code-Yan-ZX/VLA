...
\begin{table*}[t]
  ...
  identical within every cell. Its GQA $-0.4$~pp difference is a tie. Paired
  McNemar statistics were not run for InternVL3, so no $z$ is printed for that
  block.
\end{table*}
...
\begin{table}[t]
  ...
  \caption{Fourth-family gate --- GLM-4.1V-9B-Thinking (Zhipu, 2025),
    ...
    same query-blind L2 selector, greedy deterministic decoding, and
    model-default resolution (effective max $\approx 4.82$M px). Pre and post
    prompt token ids are identical per sample. $\Delta =$ pre $-$ post. This is
    a gate, not a full split; GQA is protocol-inconclusive under greedy
    decoding, and cross-family absolute values are not comparable.}
  \label{tab:glm}
