...
of Tables~\ref{tab:tab1}--\ref{tab:tab2}, and the gate supports the
generality of the workload-conditional stage law on a fourth lineage;
...
% --- GLM fourth-family gate table. Defined here (after the efficiency table)
% so that the compiled numbers of the existing tables --- referenced by the
% hardcoded subsection title "(Table 3)" and by supp.tex's "Table 1--5"
% naming --- stay unchanged; this table becomes the last-numbered table.
% First references: \S\ref{sec:internvl} (fourth-family gate) and the
% Discussion limitations.
\begin{table}[t]
  \centering
  \small
  \caption{Fourth-family gate --- GLM-4.1V-9B-Thinking (Zhipu, 2025),
    $n = 200$, 25\% retention ($r = 0.75$), none / pre (RBM) / post under the
    same query-blind L2 selector (iso-selector), greedy deterministic
    decoding, model-default resolution (effective max $\approx 4.82$M px),
    unified thinking-answer extraction identical across arms, iso-token exact
    per sample (pre $\equiv$ post prompt token ids). $\Delta =$ pre $-$ post.
    The GQA arm is protocol-inconclusive (floor collapse under greedy
    decoding; \S\ref{sec:discussion}, item~6); cross-family absolute values
    are not comparable (pixel cap and decoding protocol differ).}
  \label{tab:glm}
...
(Table~\ref{tab:tab3}), and post-stage
