...
RBM rule, map its trade-off against FastV, and show that four prespecified
...
\begin{figure*}[t]
  ...
  \caption{\textbf{Rank-Before-Merge (RBM) changes the selection stage while keeping the native VLM path intact.}
  On one OCRBench airport-board example (ground truth: \texttt{A105-108}),
  the left panel shows the query and answer-bearing input region. The central
  panel contrasts RBM, which ranks raw $2\!\times\!2$ units before native
  merging, with a post-merger control that ranks already mixed tokens at the
  same $\kappa=25\%$ budget. The ranking summary and measured RBM result at
  right show that the counter identifier is retained and answered correctly;
  Post-L2 and FastV-k3 fail on this case. The final stack emphasizes that RBM
  is a stage change inserted into the existing ViT--merger--LLM pipeline, not
  a second learned module. This example illustrates the OCR-heavy regime
  rather than a universal ordering.}
  \Description{A code-drawn VLM framework diagram shows an airport-board input,
  pre-merger RBM ranking versus post-merger ranking at equal retention, a
  ranking summary, a correct A105-108 result, and the accumulated ViT, RBM,
  merger, and LLM stages.}
  \label{fig:fig1}
\end{figure*}
...
We prespecified four query-aware, regime-aware, or composite extensions and
report all four as failures on Qwen3-VL-8B. \textbf{(a) Image-level routing:}
...
no stage-specific serving cost, and four prespecified Qwen3-VL extensions fail
