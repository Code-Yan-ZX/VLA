...
**Novelty monitoring.** The 0/37 gap was re-verified 2026-07-03 (keywords exhausted); on 2026-07-13 we primary-source-checked two neighbors flagged as possible threats. **RTP-LLM** [arXiv:2605.29639] (Alibaba inference engine) reports 1.86–2.52× multimodal throughput vs vLLM/SGLang via prefill–decode disaggregation, a hierarchical KV cache, and decoupled multimodal processing — but a full-text check confirms it has **no visual-token pruner, no prune-rate sweep, and no goodput-as-own-metric**; it does not close the gap. (An earlier secondary source mis-attributed "attention-entropy visual-token pruning" to RTP-LLM; that description belongs to a *separate* method, **ERA** [arXiv:2606.31982], not to RTP-LLM.) **CARES** [arXiv:2510.19496, ACL'26] performs per-image, content+query-driven resolution selection — a content-adaptive visual-token budget — but is **offline/accuracy-only** (FLOPs/token reduction on offline benchmarks; no serving-engine integration). Neither measures served throughput of a post-hoc visual-token compressor inside a continuous-batching engine; the gap stands.
