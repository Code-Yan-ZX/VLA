...
%
%  FIGURES: three placeholders (\fbox) below. To embed the finished figure,
%   replace the \fbox{...} line with \includegraphics[width=...]{...} -- see the
%   per-figure comment. Place rendered PDFs under drafts/figs/figN.pdf.
%
...
A no-merger negative control --- LLaVA-1.5-7B, whose per-token MLP projector has
no spatial 2x2 merger --- shows \emph{no} significant stage effect (TextVQA
$-2.17$~pp, McNemar $p{=}0.125$; GQA $-0.5$~pp, $p{=}1.0$; vs the merger-equipped
Qwen3-VL's $+38.4$~pp, $p{\approx}5{\times}10^{-5}$), confirming the lossy spatial
merger is \emph{necessary} for the pre$>$post effect --- without it the trend
reverses (post$\gtrsim$pre, n.s.). This turns the stage law into a causal
characterization: the pre$>$post effect arises iff the model has a lossy spatial
merger \emph{and} the input is text-dense.
\textbf{(3) A closed design space and a regime map.} Three pre-registered
extensions --- QA-gate, hybrid mask, and image router --- all
fail on the families/configurations tested (measured on Qwen3-VL): none improves
plain RBM. The method, Rank-Before-Merge (RBM), is
...
FastV wins Qwen3-VL TextVQA/GQA, while DocVQA is inconclusive,
while RBM uniquely retains dense OCR --- and 25\%-retention compression is
...
\textbf{A bounded design space.} Three pre-registered extensions that would make
pre-merger selection query-aware or regime-aware --- a query-level
embedding gate, a merger-aware hybrid mask, and an image-level router --- each fail. Together they imply that, within
the families/configurations tested, none improves plain RBM; the
query-dependent headroom (which FastV
...
\begin{enumerate}
  ...
  \item \textbf{A closed design space, a regime map, and a robust default.}
    Three pre-registered extensions (QA-gate, hybrid mask, and image router)
    all fail on the families/configurations tested (measured on Qwen3-VL), so
    none improves plain RBM within the tested scope;
    ...
    FastV wins its reference-grounded home regime, while RBM is the
...
% FIG 1 --- pipeline schematic (to be hand-drawn by the user; placeholder only).
% To embed the finished figure, replace the \fbox{...} block below with:
%     \includegraphics[width=\columnwidth]{../figs/fig1.pdf}
% and place the rendered file at drafts/figs/fig1.pdf.
\begin{figure}[t]
  \centering
  \fbox{\parbox{0.92\columnwidth}{\centering\vspace{1.6em}\textbf{[FIG:1 ---
      to be hand-drawn; embed \texttt{drafts/figs/fig1.pdf}]}\\[2pt]
    \footnotesize two-panel pipeline schematic of the stage axis\vspace{1.6em}}}
  \caption{The stage axis (iso-model / iso-token / iso-selector).
    \textbf{(a)}~RBM: ViT $\to$ score merger-\emph{input} $2\!\times\!2$-unit
    features (L2) $\to$ keep top-$\kappa$ units $\to$ native merger on survivors
    only $\to$ LLM. \textbf{(b)}~Post: ViT $\to$ native merger on \emph{all}
    units $\to$ score merger-\emph{output} features $\to$ keep top-$\kappa$
    $\to$ LLM (dashed variant: FastV prunes at LLM layer~2). The native-merger
    and LLM glyphs are drawn identically in both panels; only the tap point of
    the saliency score differs.}
  \label{fig:fig1}
\end{figure}
...
\begin{table*}[t]
  ...
  \caption{Official metrics, full splits ($n = 5000 / 5349 / 1000 / 12578$),
    ...
    \dag\ marks the (only) significant post-stage lead (GQA); these use the
    ...
    gives $0.0$~pp on GQA). Parenthetical stderrs
...
\textbf{A no-merger negative control (LLaVA-1.5).} The stage effect requires a
lossy \emph{spatial} merger. LLaVA-1.5-7B has a per-token MLP projector
...
Without a spatial merger the trend reverses (post$\gtrsim$pre, n.s.): ranking in
the LLM space is at least as good as in the ViT space when no merger destroys
text. The lossy spatial merger is therefore \emph{necessary} for the pre$>$post
stage effect --- a causal characterization (the effect arises iff the model has a
lossy spatial merger \emph{and} the input is text-dense), not merely an
amplifier.
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}l r r r l l r@{}}
    ...
    GQA (exact match) & 0.6293 & 0.5993          & 0.6031 & $-0.4$ pp (n.s.)   & $-3.0$ pp  & 690 \\
    \bottomrule
  \end{tabular}
  ...
  none. (iii)~The GQA difference is indistinguishable at full $n = 12578$
  ($-0.4$~pp; paired 95\% CI $[-0.93, +0.17]$ crosses 0, permutation $p =
  0.18$), contrasted with Qwen's apparent post edge (a deepstack[0]-input
  layer-8 ranking artifact dissolved by the pure-stage control,
  \S\ref{sec:qwen}) --- we claim no
  GQA stage lead on InternVL3, and no preregistered equivalence bound exists,
  so we do not call it a statistical tie.
\end{table}
...
just Qwen's PatchMerger), confirming the M3 mechanism is architecture-general.

% FIG 2 --- grouped bars of the pre-post Delta at 25% retention (headline figure).
% To embed the finished figure, replace the \fbox{...} block below with:
%     \includegraphics[width=\columnwidth]{../figs/fig2.pdf}
% and place the rendered file at drafts/figs/fig2.pdf.
\begin{figure}[t]
  \centering
  \fbox{\parbox{0.92\columnwidth}{\centering\vspace{1.6em}\textbf{[FIG:2 ---
      to be hand-drawn; embed \texttt{drafts/figs/fig2.pdf}]}\\[2pt]
    \footnotesize grouped bars: pre$-$post $\Delta$ at 25\% retention, three
    families $\times$ four benchmarks\vspace{1.6em}}}
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}ll r r r l@{}}
    ...
    \multirow{4}{*}{Qwen3-VL-8B}
      ...
      & DocVQA (dev; 600k cap)       & 200   & 0.5863          & 0.5924 & $+0.6$ pp (RBM)$^{\ddagger}$ \\
    ...
    \multirow{4}{*}{Qwen2.5-VL-7B}
      ...
      & GQA (dev; default)           & 200   & 0.475           & 0.520 & $+4.5$ pp (RBM)$^{\ddagger}$ \\
      & DocVQA (dev; 600k cap)       & 200   & 0.4852          & 0.5062 & $+2.1$ pp (RBM)$^{\ddagger}$ \\
...
attention has mixed in the question) leads RBM on Qwen3-VL TextVQA and GQA
($+17.2 / +8.9$~pp), while Qwen3-VL DocVQA is statistically indistinguishable
(RBM 0.5924 vs FastV 0.5863; paired difference inconclusive). FastV also leads
on Qwen2.5-VL TextVQA ($+7.8$~pp)
...
% FIG 3 --- retention-vs-gap curves (compression-activated effect).
% To embed the finished figure, replace the \fbox{...} block below with:
%     \includegraphics[width=\columnwidth]{../figs/fig3.pdf}
% and place the rendered file at drafts/figs/fig3.pdf.
\begin{figure}[t]
  \centering
  \fbox{\parbox{0.92\columnwidth}{\centering\vspace{1.6em}\textbf{[FIG:3 ---
      to be hand-drawn; embed \texttt{drafts/figs/fig3.pdf}]}\\[2pt]
    \footnotesize retention-vs-gap curves, $2\!\times\!2$ panels (TextVQA,
    DocVQA $\times$ Qwen3-VL, Qwen2.5-VL)\vspace{1.6em}}}
...
We pre-registered three extensions that would make pre-merger selection
query-aware or regime-aware, and report all three as failures. They
bound the tested design space and explain why RBM is frozen as
a plain query-blind method. \textbf{Evidence scope.} All three extensions below
are measured on Qwen3-VL-8B; the
...
sample-/query-level~(a), and hybrid text-routing trades OCR for nothing~(b).
\textbf{Within the families/configurations tested, the evaluated pre-merger
extensions do not improve plain RBM.} The
...
dense OCR is absent; do \textbf{not} route at runtime --- fix the
...
advantage is purely an accuracy/robustness effect. A 5-repeat repeatability
measurement (mean$\pm$std) confirms this: pre vs post req/s within $3\%$ at every
budget ($+0.04\%$ at 75\%-keep, $+2.64\%$ at 50\%-keep above the repeat-noise
floor, $-1.41\%$ at 25\%-keep), no consistent direction. \textbf{Compression scaling.}
...
practical consequence is a deliberately minimal default, RBM, plus a bounded
design space within the tested scope: three pre-registered modifications on
Qwen3-VL do not improve plain RBM (\S\ref{sec:negative}), and the query-dependent
...
query-conditioned strong baseline FastV leads it on Qwen3-VL TextVQA/GQA while
DocVQA is inconclusive (Table~\ref{tab:tab3}), and on Qwen GQA the apparent
...
raising throughput $+68\%$ (Qwen) / $1.8$--$2.5\times$ (InternVL3). Three
pre-registered negative results (measured on Qwen3-VL) bound the design space
within the tested scope: none improves plain RBM, and the query-dependent
headroom is reachable only after
