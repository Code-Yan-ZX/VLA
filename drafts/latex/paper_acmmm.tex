...
iso-budget control. There is no text-dense crossover at the native-resolution
...
post-stage edge (Qwen $+2.6$--$2.8$~pp) or indistinguishable (InternVL3, $-0.4$~pp).
...
$\Delta$) and an indistinguishable difference on InternVL3 ($-0.4$~pp; paired 95\% CI crosses 0) --- and no
...
\begin{enumerate}
  ...
  \item \textbf{A stage law --- the mechanism's generality.} Query-blind
    ...
    a small significant post-stage edge (Qwen) or an indistinguishable difference (InternVL3)
...
\begin{table}[t]
  ...
  0.18$), contrasted with Qwen's small significant post edge --- we claim no
...
GQA stays within the red line --- indistinguishable here ($-0.4$~pp) versus Qwen3-VL's small
