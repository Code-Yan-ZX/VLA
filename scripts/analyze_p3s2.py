...
for bench in ALL_BENCHES:
    for ck, label in [("control576", "r0 control"), ("keep288", "FastV r50")]:
        ...
        if j is None:
            continue
        out.append(f"| {bench} | {label} | {j['agg']['accuracy']:.3f} |")
...
out += ["", "## 5. Concurrency matrix (GQA, batch-submit, served-throughput scaling)",
        ...
        "| config | c1 | c4 | c12 |",
        "|---|---|---|---|"]
# c1/c12 from M2 (runs/p2_probe or p2_d), c4 from this step
def load_req_s(path):
    p = Path(path)
    if not p.exists():
        return "—"
    return f"{json.load(open(p))['agg']['served_req_s']['mean']:.3f}"
# pull c1/c12 from M2 probe if present
c1_r0 = load_req_s(ROOT / "runs/p2_probe/gqa_r0_metrics.json")
c1_r50 = load_req_s(ROOT / "runs/p2_probe/gqa_r50_metrics.json")
c12_r0 = load_req_s(ROOT / "runs/p2_d/p3s1_gqa_fixed_r25_bursty_mt32.json")  # placeholder
c4_r0 = load_req_s(D / "gqa_r0_c4_batch.json")
c4_r50 = load_req_s(D / "gqa_r50_c4_batch.json")
out.append(f"| r0 (control) | {c1_r0} | {c4_r0} | (see M2) |")
out.append(f"| r50 | {c1_r50} | {c4_r50} | (see M2) |")
