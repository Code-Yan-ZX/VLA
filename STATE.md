...
- RankBridge 从投稿正文移除（内部审计保留），避免失败的探索性 probe 分散主线；FastV 保留同后端 full OCRBench 对比。
- 图组按用户最终指定：Fig.1 v2 不变，保留 `fig2_word` OCR 案例与 `fig4_vector` token-survival maps；在 `fig3_word` 和全量柱状图之间选择证据口径更稳的 `fig3_fullsplit_controls`。旧 RD 与强 saliency-mechanism 图不进入正文。
...
## DCC 合规与验证
- 官方 `dccpaper.cls`、单栏 12pt、US Letter；总长 10 页，51 篇正文实引，无 `\nocite`。
