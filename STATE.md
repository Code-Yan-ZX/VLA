...
- **GPU chain DONE**（run_gpu_chain.sh）：D2 ✅(mixed) -> P1-1 ✅(GENERALIZES) -> P1-3 ✅(stage-neutral) -> GLM ✅. 全 4 阶段完成。
- **GLM pre/post (sampling seed0) = 方向确认、绝对值仍坏**（runs/glm4v_gate_sampling/）：textvqa pre 0.255 vs post 0.060 = +19.5pp、docvqa +12.2pp、gqa +5.0pp -> pre>post 方向在 GLM（第4族，broken 集成）亦成立（greedy +16.8/+9.8 一致）。但绝对值 0.06-0.26 ≪官方 ~0.8（think loop 未修）-> GLM 仍撤出论文（option A），方向数据存档。user 令跑的 GLM 实验已完成。
