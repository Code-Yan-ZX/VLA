...
**Spine**：pre-merger 保 text/OCR 深压精度（post 灾难失败）+ workload-conditional 机制 → 暗示 adaptive stage 方法。**下一步**：强 selector + 跨架构(Qwen2.5-VL/InternVL3) + text-dense suite(OCR-Bench/DocVQA/ChartQA)+object suite(GQA/POPE/MMBench) + n=500 mean±std + vs VisionZip + 写论文。
