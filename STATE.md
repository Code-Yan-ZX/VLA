...
> 最近更新：2026-07-31 · **投稿形态仅差渲染**：acmart 转换就绪（drafts/latex/，静态自查全 PASS）+ supp 全填（S1–S9）+ bib 20 条核真。FIG:1 已产出代码版内部草稿（`drafts/figures/gen_fig1_rbm.py` → `drafts/figs/fig1.{pdf,png}`），当前遮罩为明确标注的 layout proxy。已建立 `drafts/figures/real_data_pipeline/` 和服务器 Claude Code 任务提示词，要求复用真实 Qwen3 merger-input/output L2 捕获、NPZ 校验后重绘 FIG:1，并用审计 JSON 代码绘制 FIG:2/3。图齐渲染定稿 → 二次升级 user 投稿 go/no-go。
