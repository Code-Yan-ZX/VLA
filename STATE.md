...
**Spine**：pre-merger 保 text/OCR 深压精度（post 灾难失败）+ workload-conditional 机制 → 暗示 adaptive stage 方法。**✅ suite law + DocVQA clean + VisionZip对比**：pre-merger dominant-only @25% vs post vs VisionZip-style。**VZ对比**：pre(dim-only) TextVQA 0.70 > VZ-style(dom+ctx) 0.39（VZ context tokens average-pool destructive to text → dominant-only wins）。下一步：Task 4 adaptive router + Task 5 误差棒/claim_strength。
**🔧 Task4 adaptive router probe 就绪(2026-07-21)，等 GPU 空**：runner 已 patch 输出 per-sample(id,correct,question,ptid)；`router_probe.py` + `router_probe.sh` 写好，4 条 GPU 命令(pre/post×textvqa/gqa n=200 r=0.75 l2)串行 <5min。GPU 被 sues YOLO 占 26GB 不可跑。分析脚本 CPU 测过 mock 端到端 OK。缺：4 个 per-sample cell json。下一步：GPU 空时跑 `bash src/v3_premerger/router_probe.sh` → `python src/v3_premerger/router_probe.py`。
