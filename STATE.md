...
- 决策（按 user P0-1 规则）：停 GLM（不跑 seeds1/2、不 full split）；blocker=experiments/glm4v_sampling_blocker.md；**正文撤 GLM headline**（选项 A 移除 / C 修集成 待 user 定）。claim 级+投稿前 -> 已升级。
...
- 待办 P0-2b：paper 文本套用判决（InternVL3 GQA 措辞、Table3 降级、GLM 撤除）- 待 GLM 处理定后一并改。
