...
- 图组按用户最终指定：Fig.1 v2 不变；`fig2_word` OCR 案例提前为 Fig.2，`fig3_fullsplit_controls` 为 Fig.3，`fig4_vector` token-survival maps 为 Fig.4。旧 RD 与强 saliency-mechanism 图不进入正文。
